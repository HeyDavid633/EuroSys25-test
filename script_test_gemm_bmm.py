...
if __name__ == '__main__':
    ...
    for seq_len in [256, 512, 1024, 2048, 4096]:
        ...
        n = seq_len
        k = head_size
        batch_size = 4096/seq_len  # head_size
        
        batch_mat_A = torch.rand((int(batch_size * head_num), m, k), dtype=data_type, device=device, requires_grad=False)
        batch_mat_B = torch.rand((int(batch_size * head_num), k, n), dtype=data_type, device=device, requires_grad=False)    
