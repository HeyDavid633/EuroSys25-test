...
struct Testbed
{
    ...
    Testbed(
        Options<LayoutOutput> const &options_) : options(options_)
    {
    ...
    void compute_reference()
    {
        ...
        if (kIsColumnMajorOutput)
        {
            for (int n = 0; n < options.problem_size0.n(); ++n)
            {
                ...
                ElementInputScaleBias variance = ElementInputScaleBias(cutlass::constants::one<ElementLayernormCompute>() / cutlass::fast_sqrt(square_mean + ElementLayernormCompute(1e-6)));

                for (int m = 0; m < options.problem_size0.m(); ++m)
                {
                    reference_C0.at({m, n}) =
                        ElementOutput(((ElementInputScaleBias(reference_C0.at({m, n})) - mean) * variance) * tensor_Gamma.at({0, m}) + tensor_Beta.at({0, m}));
                }
            }
        }
        else
        {
            ...
            for (int m = 0; m < options.problem_size0.m(); ++m)
            {
                ...
                {
                    reference_C0.at({m, n}) =
                        ElementOutput(((float(reference_C0.at({m, n})) - mean) * variance) * float(tensor_Gamma.at({0, n})) + float(tensor_Beta.at({0, n})));
                }
            }
        }
    ...
    bool profile()
    {
        ...
        int64_t flops = int64_t(options.problem_size0.m()) * options.problem_size0.n() * options.problem_size0.k() * 2 + int64_t(options.problem_size1.m()) * options.problem_size1.n() * options.problem_size1.k() * 2;
