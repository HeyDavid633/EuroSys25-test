...
torch::Tensor run_syncfree_triangle_attention(
    torch::Tensor q,
    torch::Tensor k,
    torch::Tensor v);
...
PYBIND11_MODULE(syncfree_attention, m)
{
    ...
    m.def("run_syncfree_triangle_attention", &run_syncfree_triangle_attention, "fused full attention, Masked with lower triangle");
