# 2024.03.10 Mar.
#  对于当前0-14的算子可以随机选多段来进行融合，决策顺序 
#    (1)是否融合 （2）是否调度手写算子 （3）融合几段 （4）3决定 几段融合范围
#       (2) - (4)的决策融入到 16bit的数据value中
#  
#  python generate_genidx.py 16 12 256 12 64 1 1 16022
# 
#  参数情况：执行的超参数 
#   group1: 超参数  batch_size layer_num seq_len head_num head_size
#   group2: 掩码类型 mask_id
#   group3: 是否要融合 0/1
#   group4: 是否调度手写算子 -> 融合n段 -> n段融合范围   value(0 - 65535)
#           两组比较典型的 value测试： 16022 / 48790 使不使用手写算子的融合
#
import sys
import os
...
def main():
    ...
    # 翻译value ---> 是否使用手写算子 融合几段 几段的[start,end]
